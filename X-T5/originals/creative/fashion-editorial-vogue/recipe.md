# Fashion Editorial ("Vogue") — Fujifilm X-T5 (X-Trans V) · STILLS

A polished high-fashion magazine cover / editorial look: refined and luxurious, with deep rich blacks, clean controlled colour, and flawless smooth skin, crisp detail, a sophisticated cool-neutral palette. Creative design (see [creative README](../README.md)); drop cover/tear references into [`references/`](references/) to tune.

| Setting | Value |
|---|---|
| Film Simulation | PRO Neg. Hi |
| Grain Effect | Weak, Small |
| Color Chrome Effect | Weak |
| Color Chrome FX Blue | Off |
| White Balance | Daylight, −1 Red & +1 Blue |
| Dynamic Range | DR400 |
| Highlight (Tone Curve) | 0 |
| Shadow (Tone Curve) | +2 |
| Color | −1 |
| Sharpness | +1 |
| High ISO NR | −4 |
| Clarity | −1 |
| ISO | Auto, up to ISO 6400 |
| Exposure Compensation | 0 to +1/3 (typically) |

### Why
- PRO Neg. Hi: the portrait profile with slightly enhanced contrast and excellent skin: the fashion-cover base.
- Shadow +2 + DR400: deep luxurious editorial blacks while clean highlights stay protected (retouched-highlight feel).
- Color −1, CCE Weak, cool WB: controlled, sophisticated, slightly desaturated colour (not candy).
- Sharpness +1 + Clarity −1: crisp garments/detail, but Clarity −1 smooths skin flatteringly (the deliberate deviation, flawless skin is the point).
- Weak/Small grain: clean, polished, high-production.

### Tuning
- B&W couture: switch Film Sim to Acros, keep Shadow +2 / Sharpness +1, Clarity −1 → classic monochrome fashion.
- Warmer beauty look: WB +1R/−2B, Color 0.
- Video: Pro Neg Hi + WB + DR (manual) + Tone Curve + Sharpness transfer; Clarity/grain drop; use a light mist filter for skin instead.
