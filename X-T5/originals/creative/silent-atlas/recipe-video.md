...
|---|---|---|
| Film Simulation | Classic Negative | Teal-leaning shadows + hard tonality → the teal-orange split and cinematic depth of the reference |
| White Balance | Daylight, +3 Red & −2 Blue | Warms highlights/skin to amber-gold; −2B (not −3) keeps enough blue for Classic Neg's cyan lean to render skies/mist teal — *corrected from −3B after a second frame-verification pass, 2026-07-16* |
| Dynamic Range | DR200 | 1 stop highlight headroom for misty/overcast mountain skies; keeps contrast (DR400 too flat). ISO ≥ 250 |
| Tone Curve — Highlight | −1 | Soft highlight roll-off — the reference has gentle, non-clipping highlights in clouds/sky |
| Tone Curve — Shadow | +2 | Deep, rich, contrasty blacks with Classic Neg's teal cast; gives the moody cinematic depth |
| Color | +3 | Lifts Classic Neg's restrained saturation to warm-amber richness; compensates for absent CCE/CCFXB |
| Sharpness | −1 | Slightly soft, filmic — the reference uses a Glimmerglass filter for halation/softness |
| High ISO NR | −4 | Maximum texture/detail; standard recipe default |
| Interframe NR | AUTO | Movie-only temporal NR; reduces noise without in-frame softening |
