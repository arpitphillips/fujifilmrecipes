# Chrome Fog (Ross McConaghy) — Fujifilm X-T5 (X-Trans V)

Classic Chrome base. A creative atmospheric "foggy/hazy" look, an in-camera alternative to a Pro-Mist / diffusion lens filter, achieved with maximum-negative Clarity to mimic shooting through a steamy window. Fills a genuinely new niche in the bank (we had no in-camera fog/haze look). Created by Ross McConaghy.

[Source](https://www.rossandhisjpegs.com/fujifilm-recipes/chrome-fog)
...
> **X-Trans V, verified correct as-published (2026-07-16).** Ross authors on an X-Trans V-category body (proven by his Nostalgic Neg. recipe [Pumpkin Soup](../ross-pumpkin-soup/), that sim exists only on X-Processor 5 cameras). So this recipe's blue rendering already matches the X-T5; Color Chrome FX Blue stays Strong, with no IV→V drop needed. *Fallback:* Classic Chrome is a blue-affected sim, so in the unlikely event a particular frame's blues look over-deep, dropping CCFXB to Weak is the standard remedy. See [x-trans-v-and-conversion.md](../../../Knowledge/x-trans-v-and-conversion.md).
